...
export const metadata = {
  title: "Create Athlete Profile | Volunteer",
};

export default async function VolunteerCreateAthletePage() {
  const supabase = await createClient();
  const {
    data: { user },
  } = await supabase.auth.getUser();

  if (!user) redirect("/auth/login?redirectTo=/volunteer/athletes/new");
      ...
      <main className="flex-1 py-8">
        <div className="max-w-2xl mx-auto px-4 mb-6">
          ...
          <p className="text-gray-500 mt-1 text-sm">
            You are creating this Athlete ID on behalf of an athlete as a volunteer.
          </p>
        </div>
