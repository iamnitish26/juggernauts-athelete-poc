      ...
      <div className="flex items-start justify-between mb-6">
        <div>
          ...
          <div className="flex flex-wrap gap-3 mt-2 text-sm text-gray-500">
            <span className="flex items-center gap-1"><Calendar className="w-3.5 h-3.5" />{format(new Date(event.event_date), "dd MMM yyyy")}</span>
            <span className="flex items-center gap-1"><MapPin className="w-3.5 h-3.5" />{event.venue}, {event.district}</span>
            <span className="flex items-center gap-1"><Users className="w-3.5 h-3.5" />{event.sport} · {event.age_category}</span>
            <span className="flex items-center gap-1"><IndianRupee className="w-3.5 h-3.5" />{event.registration_fee > 0 ? `₹${event.registration_fee}` : "Free"}</span>
          </div>
        </div>
        <Link href={`/events/${id}`} target="_blank" className="text-xs text-[#5B21B6] hover:underline font-medium">
          View public page ↗
        </Link>
      </div>
